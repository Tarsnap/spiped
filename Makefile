...
all: cpusupport-config.h
	...
	export LDADD_POSIX=`export CC=\`which "${CC}"\`; cd libcperciva/POSIX && command -p sh posix-l.sh`;	\
	export CFLAGS_POSIX=`export CC=\`which "${CC}"\`; cd libcperciva/POSIX && command -p sh posix-cflags.sh`;	\
	. ./cpusupport-config.h;			\
	for D in ${PROGS}; do				\
